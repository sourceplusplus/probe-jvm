...
class LiveTransformer(private val source: String) : ClassFileTransformer {
    override fun transform(
        loader: ClassLoader, className: String, classBeingRedefined: Class<*>?,
        protectionDomain: ProtectionDomain, classfileBuffer: ByteArray
    ): ByteArray? {
        if (className.replace('/', '.') != source) {
            return null
        }
        ...
        val classVisitor: ClassVisitor = LiveClassVisitor(classWriter, source, classMetadata)
        classReader.accept(classVisitor, ClassReader.SKIP_FRAMES)
        return classWriter.toByteArray()
    }

    protected fun computeFlag(classReader: ClassReader): Int {
